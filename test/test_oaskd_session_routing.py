...
def test_opencode_project_session_ccb_session_id_priority(tmp_path):
    session_file = tmp_path / ".opencode-session"
    session_file.write_text("{}", encoding="utf-8")
    ...
    session2 = OpenCodeProjectSession(session_file=session_file, data=data2)
...
def test_compute_session_key_prioritizes_ccb_session_id_opencode(tmp_path):
    ...
    data = {"session_id": "ai-1716890000-12345", "pane_id": "%42", "pane_title_marker": "CCB-OpenCode"}
    session = OpenCodeProjectSession(session_file=session_file, data=data)
    key = compute_session_key(session)
    assert key == "ccb:ai-1716890000-12345"


def test_compute_session_key_falls_back_to_pane_id_opencode(tmp_path):
    session_file = tmp_path / ".opencode-session"
    session_file.write_text("{}", encoding="utf-8")

    data = {"pane_id": "%42", "pane_title_marker": "CCB-OpenCode"}
    session = OpenCodeProjectSession(session_file=session_file, data=data)
    key = compute_session_key(session)
    assert key == "opencode_pane:%42"


def test_load_project_session_with_ccb_session_id_opencode(tmp_path):
    session_file = tmp_path / ".opencode-session"
...
def test_load_project_session_ccb_session_id_no_fallback_opencode(tmp_path):
    session_file = tmp_path / ".opencode-session"
    session_file.write_text(json.dumps({
